function [VanHoveData] = ComputeVanHove(InputTrack, TimeSteps)
    arguments
        InputTrack {iscell} % Recieves a list of MSD analyzer tracks
        TimeSteps {isrow} = 1 % Recieves a row vector list of the dt steps.
    end
    % Outputs a list of Van

    % This function is a version of VanHove with preallocation added in.
    % The original algorithm failed to preallocate, which caused it to take
    % hours to run for large datasets with hundreds of
    % thousands of steps, as if a matrix changes shape you effectively have
    % to rewrite the entire variable, which takes a lot of time for large
    % matricies.
    % The basic idea-- create a cell struct with one cell for each tau 
    % value. Then, preallocate a matrix of the maximum possible length for 
    % each delta table, i.e. the length if every value is separated by 
    % dt=dtau. Multiply each matrix by 1i to make all values non-real.
    ...
    % The maximum possible size of Each VanHove Cell is 
    % the number of tracks times the maximum length of each track.
    ...
    RawVanHove = cell(1, max(TimeSteps));
    for tau=TimeSteps 
        
        RawVanHove{tau}=1i*ones(InputLength*NumTracks,4); 
                ...
                for t=1:BackStop 
                    ...
                    for dt=1:tau 
                        ...
                        if InputTrack{track}(t+dt,1) - InputTrack{track}(t,1) == tau 
                           ...
                           RawVanHove{tau}(counter,:)=[Dx,Dy,track,t]; 
                           % records ΔΧ, ΔY, track number, AND start frame
    ...
    RealIndex = zeros(1, max(TimeSteps)); % preallocate
    for tau=TimeSteps
        for k=1:length(RawVanHove{tau})
            if isreal(RawVanHove{tau}(k,1))
    ...
    VanHoveData = cell(max(TimeSteps),1);
    for tau=TimeSteps
        VanHoveData{tau}=1i*ones(RealIndex(tau),4);
    end
    
    % Now we clean up our preallocated matricies, copying only the real values
    % to NewVanHove
    ...
    for tau=TimeSteps
        ...
        for k=1:length(RawVanHove{tau})
            if isreal(RawVanHove{tau}(k,1))
                iter=iter+1;
                VanHoveData{tau}(iter,:)= RawVanHove{tau}(k,:);
            end
        end
    end
