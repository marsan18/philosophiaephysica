%% README
% This program designed to use the Angle Callibration method but only find
% the angle and skip any other processing steps to save time. 
%% IDEAS 
...
PSF_manual=img([(889-10):(889+10)],[(1503-10):(1503+10)],6);
...
img_profile(:,:,1) = sum(img, 1);
LR_img_profile(:,:,1)=sum(img_test_decon_lucy,1);
...
if length(zList)~=size(img,3) && length(zList)>0 %#ok<*ISMT>
...
for k = 1:size(img_profile,2)
    ...
    GaussFitMasked(:,k)= IterFit(x); %#ok<*SAGROW>
end
...
w_0 = .85*WL/(2*NA*.6)/cosd(theta(length(zList)));
z_R = pi*w_0^2*n_RI/WL;
[~,waist_indx] = min(masked_width_conv);
waist_indx = 11; %OVERRRIDE
...
FullProfilePlot=surf(zList, xList, LR_img_profile); %plots 3D lightsheet
FullProfilePlot.EdgeColor='interp';
FullProfilePlot.FaceColor="interp";
